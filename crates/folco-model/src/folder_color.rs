...
//!
//! [`FolderColor`] is a pure data enum: each variant maps to a target RGB
//! color and carries no rendering logic. The renderer bridges it to a color
//! target config via its own extension trait (`FolderColorExt`).
//!
//! The full list of colors with metadata can be serialized to JSON via
//! [`FolderColor::all_with_metadata`] for a frontend color picker.
//!
//! ```
//! use folco_model::folder_color::FolderColor;
//!
//! let color = FolderColor::Red;
//! assert_eq!(color.rgb(), (244, 67, 54));
//! ```
